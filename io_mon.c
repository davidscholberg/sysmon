...
void *io_mon( void *thread_id )
{
    // open /proc/stat and read values from it
    FILE *procstat = fopen( "/proc/stat", "r" );
    if( procstat == NULL ) 
    {
        die( "couldn't open /proc/stat in io_mon" );
    }
    ...
    if( values_read != CPU_STAT_LEN )
    {
        if( ferror( procstat ) )
        {
            die( "first fscanf on /proc/stat in io_mon failed" );
        }
        else
        {
            die_sans_errno(
                "matching error for first fscanf on /proc/stat in io_mon" );
        }
    }
    ...
    if( values_read != CPU_STAT_LEN )
    {
        if( ferror( procstat ) )
        {
            die( "second fscanf on /proc/stat in io_mon failed" );
        }
        else
        {
            die_sans_errno(
                "matching error for second fscanf on /proc/stat in io_mon" );
        }
    }
    ...
    {
        die( "couldn't close /proc/stat in io_mon" );
    }
    ...
    // calculate percent iowait
    double percent_iowait
        = ( ( double ) ( end_stats[IOWAIT] - start_stats[IOWAIT] )
    ...
    int bytes_written = snprintf( out_buffer, BUFFER_LEN, "io:%3.0f%%",
        percent_iowait );
    if ( bytes_written < 0 )
    {
        die_sans_errno( "snprintf in io_mon failed" );
    }
